...
FROM user_tables 
WHERE table_name IN (
  'USERS', 'POSTS', 'COMMENTS', 'LIKES', 'FOLLOWERS', 'REFRESH_TOKENS',
  'BOOKMARKS', 'NOTIFICATIONS', 'MESSAGES', 'STORIES', 'REELS', 'REEL_LIKES',
  'MEDIA_ASSETS', 'GROUPS', 'GROUP_MEMBERS', 'GROUP_MESSAGES', 'BROADCASTS',
  'BROADCAST_RECIPIENTS', 'FCM_TOKENS', 'USER_PRIVACY_SETTINGS', 'USER_HIDDEN_WORDS',
  'USER_BLOCKS', 'FOLLOW_REQUESTS', 'USER_REPORTS', 'MODERATION_ACTIONS'
)
ORDER BY table_name;
...
UNION ALL
SELECT 'Orphan Privacy Settings', COUNT(*) FROM USER_PRIVACY_SETTINGS ps WHERE NOT EXISTS (SELECT 1 FROM USERS u WHERE u.user_id = ps.user_id)
UNION ALL
SELECT 'Orphan Hidden Words', COUNT(*) FROM USER_HIDDEN_WORDS hw WHERE NOT EXISTS (SELECT 1 FROM USERS u WHERE u.user_id = hw.user_id)
UNION ALL
SELECT 'Orphan Blocks', COUNT(*) FROM USER_BLOCKS ub WHERE NOT EXISTS (SELECT 1 FROM USERS u WHERE u.user_id = ub.blocker_user_id)
UNION ALL
SELECT 'Orphan Follow Requests', COUNT(*) FROM FOLLOW_REQUESTS fr WHERE NOT EXISTS (SELECT 1 FROM USERS u WHERE u.user_id = fr.requester_user_id)
UNION ALL
SELECT 'Orphan Reports', COUNT(*) FROM USER_REPORTS ur WHERE NOT EXISTS (SELECT 1 FROM USERS u WHERE u.user_id = ur.reporter_user_id);
