...
export function errorHandler(err: any, req: Request, res: Response, _next: NextFunction) {
  ...
  // Handle Oracle-specific error codes (by numeric errorNum or string code)
  const oraErrorNum = typeof err.errorNum === 'number' ? err.errorNum : 0;
  const oraCode = typeof err.code === 'string' ? err.code : '';

  if (oraErrorNum === 1 || oraCode.includes('ORA-00001')) {
    return sendError(res, 'DUPLICATE_ENTRY', 'A record with this key already exists', 409);
  }
  if (oraErrorNum === 2291 || oraCode.includes('ORA-02291')) {
    return sendError(res, 'REFERENCED_KEY_NOT_FOUND', 'Referenced user or resource does not exist', 400);
  }
  if (oraErrorNum === 2292 || oraCode.includes('ORA-02292')) {
    return sendError(res, 'CHILD_RECORD_EXISTS', 'Cannot delete resource because related items exist', 400);
  }
  if (oraErrorNum === 1400 || oraCode.includes('ORA-01400')) {
    return sendError(res, 'NULL_NOT_ALLOWED', 'Required field cannot be empty', 400);
  }
  if (oraErrorNum === 28000 || oraErrorNum === 1017 || oraCode.includes('ORA-28000') || oraCode.includes('ORA-01017')) {
