...
import { Grid, Film, List, Trash2, Heart, Play, X, Check, Lock } from 'lucide-react';
...
export const ProfilePage: React.FC = () => {
  ...
  const userPosts = (postsRes?.data || []).filter((p) => p.userId === user?.userId && !p.isMock);
  const userReels = reelsList.filter((r) => r.userId === user?.userId && !r.isMock);
  ...
  return (
    <AppShell>
      ...
      <div className="p-4 space-y-4">
        {user.isPrivate && !isSelf && !user.isFollowing ? (
          <div className="aurora-glass rounded-2xl p-8 border border-slate-800 text-center space-y-3">
            <Lock className="w-10 h-10 text-brand-400 mx-auto" />
            <h3 className="text-sm font-bold text-white uppercase tracking-wider">This Profile is Private</h3>
            <p className="text-xs text-slate-400 max-w-xs mx-auto">Follow @{user.username} to access their public posts, media grid, and Bytes feed.</p>
          </div>
        ) : (
          <>
            ...
            <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
              ...
              >
                ...
                <span>Media Grid ({userPosts.filter((p) => p.imageUrl).length})</span>
              </button>
