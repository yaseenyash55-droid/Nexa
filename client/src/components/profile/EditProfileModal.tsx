...
import { Camera, Image as ImageIcon, Loader2, Check, AlertCircle } from 'lucide-react';
...
export const EditProfileModal: React.FC<EditProfileModalProps> = ({
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setErrorMsg(null);
      const uploadedUrl = await mediaApi.uploadFile(file, 'avatar');
  ...
  const handleCoverChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setErrorMsg(null);
      const uploadedUrl = await mediaApi.uploadFile(file, 'photo');
      setCoverImageUrl(uploadedUrl);
      setSuccessMsg('Cover banner uploaded successfully!');
    } catch (err: any) {
      setErrorMsg(err.message || 'Failed to upload cover banner');
    } finally {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center gap-4 pt-1">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Square 1:1 image recommended (JPEG, PNG, WebP)</p>
            <button
              type="button"
              onClick={() => avatarInputRef.current?.click()}
              disabled={isUploadingAvatar}
              className="mt-1.5 text-xs text-brand-400 font-semibold hover:underline"
            >
